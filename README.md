...
o uptime: time duration or time stamp since the app is running (ex: running since YYYY-MM-DD hh:mm:ss) Example: When you open a browser and navigate to http://localhost:8080/healthz it should return:
{
  "status": "OK",
  "version": "0.0.1",
...
- add cpu usage 
2. Use case – Check memory consumption and thread statistics to improve app performance and behavior
- add jvm utilization 
3. Use Case – check application server usage for optimum configuration
add app server usage 
4. Use Case – check db connection and usage
...
Looper and Jenkins can be integrated with Git with branching strategy – e.g. dev branch commit can trigger dev build. Stage release can be done with promoting build to stage env. While prod release can be done by cutting release branch and deploying in production.
